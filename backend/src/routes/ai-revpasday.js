// AI RevPASDay optimization
// Dynamic pricing for lodging + lift tickets + lessons as bundle
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISkiResortOperationsManager'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISkiResortOperationsManager. Focus area: RevPASDay optimization. ${`Dynamic pricing for lodging + lift tickets + lessons as bundle`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: RevPASDay optimization.\n${`Dynamic pricing for lodging + lift tickets + lessons as bundle`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'revpasday', context, parsed);
    res.json({ feature: 'revpasday', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[revpasday]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['revpasday']);
