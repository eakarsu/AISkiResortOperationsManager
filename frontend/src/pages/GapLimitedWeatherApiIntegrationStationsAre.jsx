...
export default function GapLimitedWeatherApiIntegrationStationsAre() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-weather-api-integration-stations-are', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>Limited weather API integration (stations are internal records)</h1>
      <p style={{ color: '#666', marginBottom: 16 }}>AISkiResortOperationsManager - Gap feature</p>
